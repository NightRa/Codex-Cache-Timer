...
- WinForms labels wrap text by default even with `AutoEllipsis`. A long title centered a clipped multi-line block and moved its visible text five pixels above a short title on the tested display. Row titles and times now render explicitly as single lines with vertical centering; titles retain end ellipses and full-title tooltips. A bitmap regression compares the same title prefix before and after extending it past the available width.
- At 192 DPI, GDI rendered the title ink below the row dot, and the fixed 148-pixel time column clipped longer `Cold` and `Running` labels. Title text is now aligned around the row midpoint, the status dot is centered to that midpoint, and the time column grows to fit its text while preserving a minimum title width.
